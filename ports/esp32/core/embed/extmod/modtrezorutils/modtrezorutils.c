...
STATIC mp_obj_t mod_trezorutils_firmware_hash(size_t n_args,
  ...
  for (int i = 0; i < FIRMWARE_SECTORS_COUNT; i++) {
	...
	#define BLAKE2_SIZE 	256
	uint8_t read[BLAKE2_SIZE] = {0};
	for(int j = 0; j < size; j += BLAKE2_SIZE){
		ensure(flash_read_nbyte(sector, j * BLAKE2_SIZE, read, BLAKE2_SIZE), NULL);
    	blake2s_Update(&ctx, read, BLAKE2_SIZE);
	}
...
STATIC mp_obj_t mod_trezorutils_get_firmware_chunk(const mp_obj_t index_obj,
  ...
  ensure(flash_read_nbyte(sector, offset, buf.buf, buf.len), NULL);
...
STATIC mp_obj_t mod_trezorutils_reboot_to_bootloader() {
  ...
  esp_restart();
  //	uint32_t entry_addr = 0x40378c88;
	...
	//(*entry)();
	//extern void IRAM_ATTR call_start_cpu1(void);
