...
secbool load_image_header(const uint8_t *const data, const uint32_t magic,
  ...
  flash_read_nbyte_by_address((uint32_t)data, (uint8_t *)&hdr->magic, 4);
  if (hdr->magic != magic) return secfalse;

  flash_read_nbyte_by_address((uint32_t)data + 4, (uint8_t *)&hdr->hdrlen, 4);
  if (hdr->hdrlen != IMAGE_HEADER_SIZE) return secfalse;

  //memcpy(&hdr->expiry, data + 8, 4);
  flash_read_nbyte_by_address((uint32_t)data + 8, (uint8_t *)&hdr->expiry, 4);
  ...
  flash_read_nbyte_by_address((uint32_t)data + 12, (uint8_t *)&hdr->codelen, 4);
...
secbool read_vendor_header(const uint8_t *const data,
  ...
  flash_read_nbyte_by_address((uint32_t)data, (uint8_t *)&vhdr->magic, 4);
  if (vhdr->magic != 0x565A5254) return secfalse;  // TRZV

  //memcpy(&vhdr->hdrlen, data + 4, 4);
  flash_read_nbyte_by_address((uint32_t)data + 4, (uint8_t *)&vhdr->hdrlen, 4);
  if (vhdr->hdrlen > 64 * 1024) return secfalse;

  //memcpy(&vhdr->expiry, data + 8, 4);
  flash_read_nbyte_by_address((uint32_t)data + 8, (uint8_t *)&vhdr->expiry, 4);
  if (vhdr->expiry != 0) return secfalse;

  //memcpy(&vhdr->version, data + 12, 2);
  flash_read_nbyte_by_address((uint32_t)data + 12, (uint8_t *)&vhdr->version, 2);

  //memcpy(&vhdr->vsig_m, data + 14, 1);
  //memcpy(&vhdr->vsig_n, data + 15, 1);
  //memcpy(&vhdr->vtrust, data + 16, 2);
  flash_read_nbyte_by_address((uint32_t)data + 14, (uint8_t *)&vhdr->vsig_m, 1);
  flash_read_nbyte_by_address((uint32_t)data + 15, (uint8_t *)&vhdr->vsig_n, 1);
  flash_read_nbyte_by_address((uint32_t)data + 16, (uint8_t *)&vhdr->vtrust, 2);
  ...
  flash_read_nbyte_by_address((uint32_t)data + 32 + vhdr->vsig_n * 32, (uint8_t *)&vhdr->vstr_len, 1);

  //vhdr->vstr = (const char *)(data + 32 + vhdr->vsig_n * 32 + 1);
  flash_read_nbyte_by_address((uint32_t)data + 32 + vhdr->vsig_n * 32 + 1, (uint8_t *)&vhdr->vstr, 1);
  ...
  flash_read_nbyte_by_address((uint32_t)data + vhdr->hdrlen - IMAGE_SIG_SIZE, (uint8_t *)&vhdr->sigmask, 1);

  //memcpy(vhdr->sig, data + vhdr->hdrlen - IMAGE_SIG_SIZE + 1,
  //       IMAGE_SIG_SIZE - 1);
  flash_read_nbyte_by_address((uint32_t)data + vhdr->hdrlen - IMAGE_SIG_SIZE + 1, (uint8_t *)&vhdr->sig, IMAGE_SIG_SIZE - 1);
