...
#define usb_device_descriptor_t  tusb_desc_device_t			//ypl add
...
void tinyusb_init(const usb_dev_info_t *dev_info) {
  ...
  string_descriptor[0] = language_id;          // 0: is supported language is English (0x0409)
  string_descriptor[1] = usb_str_table.manufacturer;		// 1: Manufacturer
  string_descriptor[2] = usb_str_table.product;   		  // 2: Product
  string_descriptor[3] = usb_str_table.serial_number; 	// 3: Serials, should use chip ID
  string_descriptor[4] = ""; 							// 4: configuration string
  string_descriptor[5] = usb_str_table.interface; 		// 5: interface string
...
/*软件模拟USB插拔*/
void Simulate_USB_unplug(void){
	gpio_set_level(USB_DP_GPIO_NUM, 0);
    gpio_config_t dp_gpio_config = {
        .mode = GPIO_MODE_OUTPUT,
        .pin_bit_mask = 1ULL << USB_DP_GPIO_NUM
    };
    ESP_ERROR_CHECK(gpio_config(&dp_gpio_config));

	vTaskDelay(pdMS_TO_TICKS(50));
	
	gpio_set_level(USB_DP_GPIO_NUM, 1);
...
void usb_start(void) { 
	tinyusb_config_t tusb_cfg = {
		.descriptor = &usb_dev_desc,
		.string_descriptor = string_descriptor,
		.external_phy = false,
		.configuration_descriptor = usb_config_buf,
	};
	...
	ensure(sectrue * (ESP_OK == tinyusb_driver_install(&tusb_cfg)), NULL);
}

void usb_stop(void) { tud_disconnect(); /*USBD_Stop(&usb_dev_handle); */ }
...
*/
static uint8_t *usb_get_bos_descriptor(void) {
...
static uint8_t *usb_class_get_usrstr_desc(uint8_t index, uint16_t *length) {
...
//     uint16_t length = 0;
//     return usb_class_get_cfg_desc(&length);
// }


uint8_t const * tud_descriptor_bos_cb(void)
{
  return usb_get_bos_descriptor();
}
...
bool tud_vendor_control_xfer_cb(uint8_t rhport, uint8_t stage, tusb_control_request_t const * request)
{
  ...
  {
    case TUSB_REQ_TYPE_VENDOR:			//供应商设备请求
      switch (request->bRequest)
      {
        case USB_WEBUSB_VENDOR_CODE:
			if(request->bmRequestType_bit.recipient == USB_REQ_RECIPIENT_DEVICE &&
				request->wIndex == USB_WEBUSB_REQ_GET_URL && request->wValue == USB_WEBUSB_LANDING_PAGE){
				...
				static const tusb_desc_webusb_url_t desc_url =
				{
				  ...
				  .bScheme		   = USB_WEBUSB_URL_SCHEME_HTTPS, // 0: http, 1: https
				  .url			   = URL
				};
			...
			if(request->bmRequestType_bit.recipient == USB_REQ_RECIPIENT_DEVICE &&
				request->wIndex == USB_WINUSB_REQ_GET_COMPATIBLE_ID_FEATURE_DESCRIPTOR){
				static const uint8_t winusb_wcid[] = {
				   // header
				   0x28, 0x00, 0x00, 0x00,					  // dwLength
				   0x00, 0x01,								  // bcdVersion
				...
				wait_random();
				return tud_control_xfer(rhport, request, (void*) winusb_wcid, sizeof(winusb_wcid));
			}
				
			if(request->bmRequestType_bit.recipient == USB_REQ_RECIPIENT_INTERFACE && 
				request->wIndex == USB_WINUSB_REQ_GET_EXTENDED_PROPERTIES_OS_FEATURE_DESCRIPTOR &&
				(request->wValue & 0xFF) == 0){
				static const uint8_t winusb_guid[] = {
					// header
					0x92, 0x00, 0x00, 0x00,  // dwLength
					0x00, 0x01, 			 // bcdVersion
					...
					'D', 0x00, 's', 0x00, 0x00, 0x00,  // .name
					0x50, 0x00, 0x00, 0x00, 		   // dwPropertyDataLength
					'{', 0x00, '5', 0x00, 'A', 0x00, 'B', 0x00, 'D', 0x00, 'B', 0x00,
					'E', 0x00, 'C', 0x00, '8', 0x00, '-', 0x00, '4', 0x00, 'D', 0x00,
					'3', 0x00, 'E', 0x00, '-', 0x00, '6', 0x00, '6', 0x00, '4', 0x00,
					'4', 0x00, '-', 0x00, 'D', 0x00, '7', 0x00, '8', 0x00, '8', 0x00,
					'-', 0x00, 'E', 0x00, 'B', 0x00, '7', 0x00, 'E', 0x00, '2', 0x00,
					'7', 0x00, '3', 0x00, '0', 0x00, 'A', 0x00, '1', 0x00, '2', 0x00,
					'7', 0x00, '}', 0x00, 0x00, 0x00, 0x00, 0x00,  // propertyData
				};
				wait_random();
				return tud_control_xfer(rhport, request, (void*) winusb_guid, sizeof(winusb_guid));
			}
		...
		case VENDOR_REQUEST_MICROSOFT:
		  if ( request->wIndex == 7 )
		  {
			static const uint8_t desc_ms_os_20[] = {
				...
				0x50, 0x00, 0x7b, 0x00, 0x39, 0x00, 0x37, 0x00, 0x35, 0x00, 0x46, 0x00, 0x34, 0x00, 0x34, 0x00,
				0x44, 0x00, 0x39, 0x00, 0x2d, 0x00, 0x30, 0x00, 0x44, 0x00, 0x30, 0x00, 0x38, 0x00, 0x2d, 0x00,
				0x34, 0x00, 0x33, 0x00, 0x46, 0x00, 0x44, 0x00, 0x2d, 0x00, 0x38, 0x00, 0x42, 0x00, 0x33, 0x00,
				0x45, 0x00, 0x2d, 0x00, 0x31, 0x00, 0x32, 0x00, 0x37, 0x00, 0x43, 0x00, 0x41, 0x00, 0x38, 0x00,
				0x41, 0x00, 0x46, 0x00, 0x46, 0x00, 0x46, 0x00, 0x39, 0x00, 0x44, 0x00, 0x7d, 0x00, 0x00, 0x00,
				0x00, 0x00
			};		  
