...
static void erase_sector(uint8_t sector, secbool set_magic) {
  ...
  uint8_t *ptr = (uint8_t *)header_backup;
  ensure(flash_read_nbyte_by_address((uint32_t)sector_start, ptr, NORCOW_HEADER_LEN), NULL);
...
static secbool read_item(uint8_t sector, uint32_t offset, uint16_t *key,
  ...
  *pos = offset;
  ...
  ensure(flash_read_nbyte(sector, *pos, read, 2), NULL);
  *key = read[0] + (read[1] << 8);
...
static secbool find_start_offset(uint8_t sector, uint32_t *offset,
  ...
  ensure(flash_read_word(sector, NORCOW_HEADER_LEN, &magic_value), NULL);
  ensure(flash_read_word(sector, NORCOW_HEADER_LEN + 4, &version_value), NULL);
  
  if (magic_value == NORCOW_MAGIC) {
    *offset = NORCOW_STORAGE_START;
    *version = ~(version_value);
  } else if (magic_value == NORCOW_MAGIC_V0) {
