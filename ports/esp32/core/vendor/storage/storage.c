...
static secbool auth_init(void) {
  ...
  hmac_sha256_(cached_sak, SAK_SIZE, authentication_sum,
...
static secbool auth_update(uint16_t key) {
  ...
  uint8_t tag[SHA256_DIGEST_LENGTH] = {0};
  hmac_sha256_(cached_sak, SAK_SIZE, (uint8_t *)&key, sizeof(key), tag);
...
static secbool set_wipe_code(const uint8_t *wipe_code, size_t wipe_code_len) {
  ...
  hmac_sha256_(salt, WIPE_CODE_SALT_SIZE, wipe_code, wipe_code_len, tag);
...
static secbool is_not_wipe_code(const uint8_t *pin, size_t pin_len) {
  ...
  hmac_sha256_(salt, WIPE_CODE_SALT_SIZE, wipe_code, wipe_code_len,
              computed_tag1);
  ...
  wait_random();
  hmac_sha256_(salt, WIPE_CODE_SALT_SIZE, pin, pin_len, computed_tag1);

  // Recompute to check for fault injection attack.
  wait_random();
  hmac_sha256_(salt, WIPE_CODE_SALT_SIZE, pin, pin_len, computed_tag2);
...
*/
  ...
  if(len > 0){
	  ensure(flash_read_nbyte_by_address((uint32_t)val, &value, 1), NULL);
	  if(value != FALSE_BYTE){
	  	return secfalse;
	  }
  }
...
static secbool v0_pin_get_fails(uint32_t *ctr) {
  ...
  if (secfalse != norcow_get(V0_PIN_FAIL_KEY, &val, &len)) {
    for (unsigned int i = 0; i < len / sizeof(uint32_t); i++) {
      ...
      uint32_t word;
      ensure(flash_read_nbyte_by_address((uint32_t)val + i * 4, (uint8_t*)&word, 4), NULL);
      if (word != 0) {
...
static secbool storage_upgrade(void) {
  ...
  if (norcow_active_version == 0) {
	...
	uint32_t read;
	if(sectrue == found){
		ensure(flash_read_nbyte_by_address((uint32_t)val, (uint8_t *)&read, 4), NULL);
	}
		
    if (sectrue == found && read != V0_PIN_EMPTY) {
