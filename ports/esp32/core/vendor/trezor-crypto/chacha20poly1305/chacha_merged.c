...
static DRAM_ATTR char sigma[16] = "expand 32-byte k";
static DRAM_ATTR char tau[16] = "expand 16-byte k";

IRAM_ATTR void ECRYPT_keysetup(ECRYPT_ctx *x,const u8 *k,u32 kbits,u32 ivbits)
{
  (void)ivbits;
...
IRAM_ATTR void ECRYPT_ivsetup(ECRYPT_ctx *x,const u8 *iv)
{
  x->input[12] = 0;
  x->input[13] = 0;
...
IRAM_ATTR void ECRYPT_ctrsetup(ECRYPT_ctx *x,const u8 *ctr)
{
  x->input[12] = U8TO32_LITTLE(ctr + 0);
  x->input[13] = U8TO32_LITTLE(ctr + 4);
}

IRAM_ATTR void ECRYPT_encrypt_bytes(ECRYPT_ctx *x,const u8 *m,u8 *c,u32 bytes)
{
...
IRAM_ATTR void ECRYPT_keystream_bytes(ECRYPT_ctx *x,u8 *stream,u32 bytes)
{
  u32 i = 0;
